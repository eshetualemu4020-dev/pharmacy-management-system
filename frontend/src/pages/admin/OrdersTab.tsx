  ...
  return (
    <div className="flex-1 p-10 overflow-y-auto">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-bold text-main mb-2">Order Management</h1>
          <p className="text-muted">Monitor and manage customer orders lifecycle.</p>
        </div>
      </div>
      ...
      {/* Orders Table */}
      <div className="bg-surface rounded-2xl border border-subtle overflow-hidden shadow-xl">
        {loading ? (
          <div className="p-10 text-center text-muted">Loading orders...</div>
        ) : error ? (
          <div className="p-10 text-center text-red-400">{error}</div>
        ) : orders.length === 0 ? (
          <div className="p-10 text-center text-muted">No orders found.</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-hover border-b border-subtle">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider">Items</th>
                  <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider">Total</th>
                  <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider">Payment</th>
                  <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider">Order Status</th>
                ...
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-hover transition-colors">
                    <td className="px-6 py-4">
                      <span className="font-semibold text-main">ORD-{order.id.toString().padStart(4, '0')}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-main">{order.customer_name}</div>
                      <div className="text-xs text-muted">{order.customer_email}</div>
                    </td>
                    <td className="px-6 py-4 text-muted text-sm">
                      {new Date(order.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 text-main font-medium">{order.total_items}</td>
                    <td className="px-6 py-4 text-main font-semibold">{formatCurrency(order.total_amount)}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button 
                        ...
                        className="p-2 bg-hover hover:bg-[#9b51e0]/20 text-muted hover:text-[#9b51e0] rounded-lg transition-colors"
                        title="View Details"
                      >
