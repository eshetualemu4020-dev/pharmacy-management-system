...
export default function OrderDetailsModal({ orderId, onClose, onUpdate }: OrderDetailsModalProps) {
  ...
  if (loading) return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-surface p-8 rounded-2xl border border-subtle shadow-2xl flex flex-col items-center">
        <div className="w-10 h-10 border-4 border-[#9b51e0] border-t-transparent rounded-full animate-spin mb-4"></div>
        <p className="text-main font-medium">Loading details...</p>
      </div>
    </div>
  );
  ...
  if (error || !order) return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-surface p-8 rounded-2xl border border-subtle shadow-2xl w-full max-w-md">
        <div className="text-center">
          <XCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-main mb-2">Error</h2>
          <p className="text-muted mb-6">{error || 'Order not found'}</p>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-end">
      <div className="bg-base w-full max-w-4xl h-full shadow-2xl flex flex-col animate-slide-in-right border-l border-subtle">
        
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b border-subtle bg-surface">
          <div>
            ...
            <p className="text-muted mt-1 text-sm flex items-center">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {!['completed', 'cancelled', 'delivered'].includes(order.status) && (
            <div className="bg-gradient-to-r from-[#232136] to-[#1e1c2e] p-6 rounded-2xl border border-subtle flex flex-wrap gap-4 items-center justify-between">
              <div>
                <h3 className="text-main font-medium mb-1">Update Order Status</h3>
                <p className="text-muted text-sm">Advance this order through the fulfillment lifecycle.</p>
          ...
          {showCancelPrompt && (
            <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl">
              ...
              <p className="text-muted mb-4 text-sm">Are you sure you want to cancel this order? This action cannot be undone.</p>
              <textarea
                value={cancelReason}
                onChange={(e) => setCancelReason(e.target.value)}
                placeholder="Reason for cancellation (required)"
                className="w-full bg-base border border-subtle-hover rounded-xl p-3 text-main focus:outline-none focus:border-red-400 mb-4"
                rows={3}
              />
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowCancelPrompt(false)} className="px-4 py-2 text-main hover:bg-hover rounded-lg transition-colors">
                  Keep Order
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-surface p-6 rounded-2xl border border-subtle">
              ...
              <div className="space-y-3">
                <div><span className="text-muted text-sm">Name:</span> <p className="text-main font-medium">{order.customer_name}</p></div>
                <div><span className="text-muted text-sm">Email:</span> <p className="text-main">{order.customer_email}</p></div>
                <div><span className="text-muted text-sm">Phone:</span> <p className="text-main">{order.customer_phone}</p></div>
              </div>
            </div>
            ...
            <div className="bg-surface p-6 rounded-2xl border border-subtle">
              ...
              <div className="space-y-3">
                <div><span className="text-muted text-sm">Method:</span> <p className="text-main font-medium uppercase">{order.delivery_method}</p></div>
                {order.delivery_method === 'delivery' && (
                  <div><span className="text-muted text-sm">Address:</span> <p className="text-main">{order.delivery_address || 'No address provided'}</p></div>
                )}
                {order.notes && (
                  <div><span className="text-muted text-sm">Notes:</span> <p className="text-main italic">"{order.notes}"</p></div>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="bg-surface p-6 rounded-2xl border border-subtle">
              <div className="flex justify-between items-start mb-4">
                ...
                <select
                  ...
                  className={`text-sm font-bold px-3 py-1.5 rounded-lg outline-none cursor-pointer border border-subtle-hover ${
              ...
              <div className="space-y-2">
                <div className="flex justify-between"><span className="text-muted">Subtotal:</span> <span className="text-main font-medium">{formatCurrency(subtotal)}</span></div>
                <div className="flex justify-between"><span className="text-muted">Discount:</span> <span className="text-green-400 font-medium">-{formatCurrency(totalDiscount)}</span></div>
                <div className="flex justify-between pt-2 border-t border-subtle">
                  <span className="text-muted font-bold">Total Amount:</span> 
                  <span className="text-main font-bold text-lg">{formatCurrency(order.total_amount)}</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-surface rounded-2xl border border-subtle overflow-hidden">
            <h3 className="text-lg font-bold text-main p-6 border-b border-subtle">Order Items</h3>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-hover border-b border-subtle">
                    ...
                    <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider text-right">Unit Price</th>
                    <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider text-right">Quantity</th>
                    <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider text-right">Discount</th>
                    <th className="px-6 py-4 text-xs font-bold text-muted uppercase tracking-wider text-right">Subtotal</th>
                  </tr>
                </thead>
                  ...
                  {order.items.map((item: any) => (
                    <tr key={item.id} className="hover:bg-hover transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-medium text-main">{item.drug_name}</div>
                        <div className="text-xs text-muted">Batch: {item.batch_id}</div>
                      ...
                      <td className="px-6 py-4 text-main text-right">{formatCurrency(item.unit_price)}</td>
                      <td className="px-6 py-4 text-main font-medium text-right">{item.quantity}</td>
                      <td className="px-6 py-4 text-green-400 text-right">-{formatCurrency(item.discount || 0)}</td>
                      <td className="px-6 py-4 text-main font-bold text-right">{formatCurrency(((item.quantity * item.unit_price) - (item.discount || 0)))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-surface rounded-2xl border border-subtle overflow-hidden p-6">
            <h3 className="text-lg font-bold text-main mb-4">Order History</h3>
            <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
              {order.history.map((event: any, index: number) => (
                <div key={event.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-subtle-hover bg-base text-muted shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow">
                    <CheckCircle2 className="w-5 h-5 text-[#9b51e0]" />
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-hover p-4 rounded-xl border border-subtle shadow-lg">
                    <div className="flex items-center justify-between space-x-2 mb-1">
                      <div className="font-bold text-main uppercase text-sm">
                        {event.previous_status ? `${event.previous_status.replace('_', ' ')} → ` : ''}{event.new_status.replace('_', ' ')}
                      </div>
                      <time className="font-medium text-muted text-xs">{new Date(event.created_at).toLocaleString()}</time>
                    </div>
                    <div className="text-sm text-muted">
                      Changed by <span className="text-main">{event.user_name || 'System'}</span>
                    </div>
                    {event.reason && (
              ...
              {order.history.length === 0 && (
                <div className="text-muted text-sm text-center">No history recorded yet.</div>
              )}
            </div>
          </div>
