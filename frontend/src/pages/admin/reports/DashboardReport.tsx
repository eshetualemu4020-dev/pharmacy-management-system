...
export default function DashboardReport({ dateRange }: { dateRange: { startDate: string, endDate: string } }) {
  ...
  if (loading) return <div className="py-12 text-center text-muted">Loading dashboard metrics...</div>;
  if (error) return <div className="text-red-400 p-4 bg-red-500/10 rounded-xl">{error}</div>;
  if (!data) return null;

  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-main mb-4">Executive Summary</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-base p-6 rounded-2xl border border-subtle">
          ...
          <p className="text-3xl font-bold text-main">{formatCurrency(data.total_revenue)}</p>
        </div>

        <div className="bg-base p-6 rounded-2xl border border-subtle">
          <div className="flex items-center space-x-3 mb-2">
            <div className="p-2 bg-blue-500/10 rounded-lg text-blue-400"><ShoppingCart className="w-5 h-5" /></div>
            <h3 className="text-muted font-medium">Online Orders</h3>
          </div>
          <p className="text-3xl font-bold text-main">{data.total_orders}</p>
        ...
        <div className="bg-base p-6 rounded-2xl border border-subtle">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h3 className="text-muted font-medium">In-Store Sales</h3>
          </div>
          <p className="text-3xl font-bold text-main">{data.total_sales}</p>
        </div>

        <div className="bg-base p-6 rounded-2xl border border-subtle">
          <div className="flex items-center space-x-3 mb-2">
            <div className="p-2 bg-purple-500/10 rounded-lg text-purple-400"><FileText className="w-5 h-5" /></div>
            <h3 className="text-muted font-medium">Pending Rx</h3>
          </div>
          <p className="text-3xl font-bold text-main">{data.pending_prescriptions}</p>
        </div>
      </div>

      <div className="bg-base p-6 rounded-2xl border border-subtle">
        <h3 className="text-lg font-bold text-main mb-4 flex items-center space-x-2">
          <AlertTriangle className="w-5 h-5 text-amber-400" />
          <span>Inventory Alerts</span>
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-surface rounded-xl border border-amber-500/20">
            <p className="text-muted text-sm mb-1">Low Stock Products</p>
            <p className="text-2xl font-bold text-amber-400">{data.low_stock_products}</p>
          </div>
          <div className="p-4 bg-surface rounded-xl border border-orange-500/20">
            <p className="text-muted text-sm mb-1">Expiring Soon (90 Days)</p>
            <p className="text-2xl font-bold text-orange-400">{data.expiring_soon}</p>
          </div>
          <div className="p-4 bg-surface rounded-xl border border-red-500/20">
            <p className="text-muted text-sm mb-1">Expired Products</p>
