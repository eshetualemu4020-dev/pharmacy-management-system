...
export default function OrdersReport({ dateRange }: { dateRange: { startDate: string, endDate: string } }) {
  ...
  if (loading) return <div className="text-muted py-8 text-center">Loading orders report...</div>;
  if (!data) return null;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-base p-4 rounded-2xl border border-subtle text-center">
          <p className="text-muted text-xs uppercase font-bold mb-1 tracking-wider">Total Orders</p>
          <p className="text-2xl font-bold text-main">{summary.total_orders}</p>
        </div>
        <div className="bg-base p-4 rounded-2xl border border-subtle text-center">
          <p className="text-muted text-xs uppercase font-bold mb-1 tracking-wider">Total Value</p>
          <p className="text-2xl font-bold text-main">{formatCurrency(summary.total_order_value)}</p>
        </div>
        ...
        <div className="bg-base p-4 rounded-2xl border border-blue-500/20 text-center">
      ...
      <div className="bg-base p-6 rounded-2xl border border-subtle">
        <h3 className="text-lg font-bold text-main mb-6">Order Volume Trend</h3>
        <div className="h-80 w-full">
          {ordersByDate.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full text-muted">No orders for this period</div>
          )}
        </div>
      </div>
    </div>
