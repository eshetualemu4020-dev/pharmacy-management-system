...
export default function PrescriptionsReport({ dateRange }: { dateRange: { startDate: string, endDate: string } }) {
  ...
  if (loading) return <div className="text-[#a09eb5] py-8 text-center">Loading prescriptions report...</div>;
  if (!data) return null;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-[#110f22] p-4 rounded-2xl border border-white/5 text-center">
          <p className="text-[#a09eb5] text-xs uppercase font-bold mb-1 tracking-wider">Total Rx</p>
          <p className="text-2xl font-bold text-white">{summary.total_prescriptions}</p>
        </div>
        ...
        <div className="bg-[#110f22] p-4 rounded-2xl border border-white/5 text-center">
          <p className="text-[#a09eb5] text-xs uppercase font-bold mb-1 tracking-wider">Approval Rate</p>
          <p className="text-2xl font-bold text-white">{summary.approval_rate}%</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <h3 className="text-lg font-bold text-white mb-6">Prescription Volume</h3>
          <div className="h-64 w-full">
            {prescriptionsByDate.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-[#a09eb5]">No data for this period</div>
            )}
          </div>
        </div>

        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <h3 className="text-lg font-bold text-white mb-4">Reviews by Pharmacist</h3>
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {prescriptionsByUser.map((u: any, i: number) => (
                <tr key={i}>
                  <td className="py-3 text-white capitalize">{u.username || 'System'}</td>
                  <td className="py-3 text-white font-medium text-right">{u.count}</td>
                </tr>
              ))}
              {prescriptionsByUser.length === 0 && (
                <tr><td colSpan={2} className="py-4 text-center text-[#a09eb5]">No reviews recorded.</td></tr>
              )}
            </tbody>
          </table>
        </div>
