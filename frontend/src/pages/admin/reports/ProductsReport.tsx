...
export default function ProductsReport({ dateRange }: { dateRange: { startDate: string, endDate: string } }) {
  ...
  if (loading) return <div className="text-[#a09eb5] py-8 text-center">Loading products report...</div>;
  if (!data) return null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#110f22] p-6 rounded-2xl border border-emerald-500/20">
          <h3 className="text-lg font-bold text-emerald-400 mb-4 flex items-center gap-2">
            <TrendingUp className="w-5 h-5" /> Top 5 Best-Selling Products
          </h3>
          <div className="space-y-4">
            {bestSelling.slice(0,5).map((p: any, i: number) => (
              <div key={i} className="flex justify-between items-center bg-[#232136] p-4 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#a09eb5]">{p.category_name || 'Uncategorized'}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-emerald-400 font-bold">{p.total_sold_qty} sold</p>
                  <p className="text-xs text-white">{formatCurrency(p.total_revenue)}</p>
                </div>
              </div>
            ))}
            {bestSelling.length === 0 && <p className="text-[#a09eb5]">No sales data.</p>}
          </div>
        </div>

        <div className="bg-[#110f22] p-6 rounded-2xl border border-red-500/20">
          ...
          <div className="space-y-4">
            ...
            {leastSelling.length === 0 && <p className="text-[#a09eb5]">No sales data.</p>}
          </div>
        </div>
      </div>

      <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
        <h3 className="text-lg font-bold text-white mb-6">Top 10 Products by Quantity</h3>
        <div className="h-80 w-full">
          {bestSelling.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full text-[#a09eb5]">No sales data</div>
          )}
        </div>
      </div>

      <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
        <h3 className="text-lg font-bold text-white mb-4">Complete Product Performance</h3>
              ...
              {performance.slice(0,50).map((p: any, i: number) => (
                <tr key={i} className="hover:bg-white/[0.02]">
                  <td className="py-3 px-4 text-white">
                    ...
                    <span className="block text-xs text-[#a09eb5]">{p.generic_name}</span>
                  </td>
                  <td className="py-3 px-4 text-white">{p.category_name || 'N/A'}</td>
                  <td className="py-3 px-4 text-right font-medium text-emerald-400">{p.total_sold_qty}</td>
                  <td className="py-3 px-4 text-right text-white">{formatCurrency(p.total_revenue)}</td>
                </tr>
              ))}
              {performance.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-[#a09eb5]">No data available.</td></tr>
              )}
            </tbody>
          </table>
          {performance.length > 50 && (
            <p className="text-center text-[#a09eb5] text-sm mt-4">Showing top 50 results.</p>
          )}
        </div>
      </div>
