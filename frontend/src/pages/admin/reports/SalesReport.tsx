...
export default function SalesReport({ dateRange }: { dateRange: { startDate: string, endDate: string } }) {
  ...
  if (loading) return <div className="text-[#a09eb5] py-8 text-center">Loading sales report...</div>;
  if (!data) return null;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <p className="text-[#a09eb5] text-sm mb-1">Total In-Store Revenue</p>
          <p className="text-2xl font-bold text-white">{formatCurrency(data.summary.total_revenue)}</p>
        </div>
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <p className="text-[#a09eb5] text-sm mb-1">Number of Sales</p>
          <p className="text-2xl font-bold text-white">{data.summary.number_of_sales}</p>
        </div>
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <p className="text-[#a09eb5] text-sm mb-1">Average Sale Value</p>
          <p className="text-2xl font-bold text-white">{formatCurrency(data.summary.average_sale_value)}</p>
        </div>
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <p className="text-[#a09eb5] text-sm mb-1">Total Discounts Given</p>
      ...
      <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
        ...
        <div className="h-80 w-full">
          {data.salesByDate.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full text-[#a09eb5]">No sales data for this period</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {data.salesByCategory.map((c: any, i: number) => (
                <tr key={i}>
                  <td className="py-3 text-white">{c.category || 'Uncategorized'}</td>
                  <td className="py-3 text-white font-medium text-right">{formatCurrency(c.revenue)}</td>
                </tr>
              ))}
              {data.salesByCategory.length === 0 && (
                <tr><td colSpan={2} className="py-4 text-center text-[#a09eb5]">No data</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="bg-[#110f22] p-6 rounded-2xl border border-white/5">
          <h3 className="text-lg font-bold text-white mb-4">Sales by Pharmacist</h3>
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 text-[#a09eb5] text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {data.salesByUser.map((u: any, i: number) => (
                <tr key={i}>
                  <td className="py-3 text-white capitalize">{u.username}</td>
